                    ...
                    double *lowerleft, double *upperright)
{
...
#ifdef SPAM
  Rprintf("Parameters: npoints %d, dim %d, rempoints %d, nforced %d\n",
          npoints, dim, rempoints, nforced);
  Rprintf("Lower: (%g", lowerleft[0]);
  for (i = 1; i < dim; i++)
    Rprintf(", %g", lowerleft[i]);
  Rprintf(")\nUpper: (%g", upperright[0]);
  for (i = 1; i < dim; i++)
    Rprintf(", %g", upperright[i]);
  Rprintf(")\nUncategorized ('forced') points are:\n");
  for (i = 0; i < nforced; i++)
  {
    Rprintf("(%g", forced[i][0]);
    for (j = 1; j < dim; j++)
      Rprintf(", %g", forced[i][j]);
    Rprintf(")\n");
  }
#endif
  ...
  for (i = 0; i < nforced; i++)
  {
    ...
    for (j = 0; j < dim; j++)
    {
      ...
      { // strictly internal
        if (status)
        {
          stop("PROBLEM: Point occurs as double-internal\n");
          // Rprintf("PROBLEM: Point occurs as double-internal\n");
          // fflush(stderr);
          // abort();
        }
        status = 1;
  ...
  for (i = 0; i < dim; i++)
    qsort(&(coordlist[i][0]), indexes[i], sizeof(double), dbl_compare);
  maxpoints[0] = indexes[0];
  for (i = 1; i < dim; i++)
    maxpoints[i] = maxpoints[i - 1] + indexes[i];

#ifdef SPAM
  Rprintf("Categorization: %d lower-left, %d internal.\n", rempoints, maxpoints[dim - 1]);
  for (i = 0; i < dim; i++)
  {
    if (!indexes[i])
    {
      Rprintf("Direction %d: Nothing.\n", i);
      continue;
    }
    Rprintf("Direction %d: %g", i, coordlist[i][0]);
    for (j = 1; j < indexes[i]; j++)
      Rprintf(", %g", coordlist[i][j]);
    Rprintf("\n");
  }
#endif
...
#ifdef SPAM
  Rprintf("Direction 0 only, biggest: ");
  for (j = 0; j <= maxpoints[0]; j++)
    Rprintf("%g ", biggest[0][j]);
  Rprintf("\nDirections 0 only, smallest: ");
  for (j = 0; j <= maxpoints[0]; j++)
    Rprintf("%g ", smallest[0][j]);
  Rprintf("\n");
#endif
...
#ifdef SPAM
    Rprintf("Directions 0--%d, biggest: ", i);
    for (j = 0; j <= maxpoints[i]; j++)
      Rprintf("%g ", biggest[i][j]);
    Rprintf("\nDirections 0--%d, smallest: ", i);
    for (j = 0; j <= maxpoints[i]; j++)
      Rprintf("%g ", smallest[i][j]);
    Rprintf("\n");
#endif
  }
  ...
  {
#ifdef WORK_OUTPUT
    Rprintf("Worse bound: %g\n", maxdiscr);
#endif
    globallower = maxdiscr;
  }
#ifdef SPAM
  else
    Rprintf("Conclusion: %g\n", maxdiscr);
#endif
  return maxdiscr;
}
                   ...
                   double *lowerleft, double *upperright)
{
  ...
  double **newforced = (double **)malloc((nforced + rempoints) * sizeof(double *));
  ...
  qsort(forced, nforced, sizeof(double *), dbl_compare);
  i = 0;
  forcedidx = 0;
...
double oydiscr(double **pointset, int dim, int npoints, double *lower)
{
  ...
  double **pre_force = (double **)malloc(2 * dim * sizeof(double *));
  double discr, *border;
  double maxcoord;
  int maxpos;
  int is_border[npoints];
  double **clone_set = (double **)malloc(npoints * sizeof(double *));
  int i, j, k;
  for (i = 0; i < dim; i++)
  {
    border = (double *)malloc(dim * sizeof(double));
    for (j = 0; j < dim; j++)
    {
      if (i == j)
        border[j] = 1.0;
